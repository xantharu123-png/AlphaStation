        ...
        for pole_start in range(max(0, n - 15), n - 4):  # Mind. 4 Kerzen nach Pole-Start
            for pole_len in range(2, min(8, n - pole_start - 1)):  # Pole: 2-7 Kerzen
                ...
                if consistency >= 0.8:
                    ...
                    _score += 5
                    _details.append(f"⚠️ Moderater Trend: {consistency*100:.0f}% der Kerzen")
                ...
                if flag_tightness <= 30:
                    ...
                    _score += 8
                    _details.append(f"⚠️ Breite Flag: {flag_tightness:.0f}% der Pole-Range")
                ...
                if retracement_pct <= 23.6:
                    ...
                    _score += 10
                    _details.append(f"⚠️ Gesundes Retracement: {retracement_pct:.1f}%")
                else:
                    _score += 3
                    _details.append(f"⚠️ Tiefes Retracement: {retracement_pct:.1f}%")
                ...
                if vol_decline <= 0.5:
                    ...
                    _score += 8
                    _details.append(f"⚠️ Volumen stabil: {vol_decline:.0%} des Pole-Vol")
                else:
        ...
        if len(body_ratios) >= 15:
            ...
            if prior_body > 0:
                ...
                if body_decline < 0.4:
                    ...
                    score += 2
                    details.append(f"⚠️ Body leicht schrumpfend: {body_decline:.2f}x")
                else:
        ...
        if len(obv) >= 6:
            ...
            if price_flat:
                if direction == "long" and obv_rising:
                    ...
                    details.append(f"⚠️ OBV neutral (kein Signal)")
            else:
                if direction == "long" and obv_rising:
                    score += 7; sm_hits += 1
...
def identify_harmonic_pattern(pivots, prices, min_pivots=5):
    ...
    for i in range(len(pivots) - 4):
        ...
        ab_retracement = min(max(abs(B - A) / xa_move, 0.0), 1.5)

        ab_move = abs(B - A)
        if ab_move == 0:
            continue
        bc_retracement = min(max(abs(C - B) / ab_move, 0.0), 1.5)
