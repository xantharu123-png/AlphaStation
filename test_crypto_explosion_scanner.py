...
def _bars(count, start=9.6, step=0.002, volume=1000, last=None):
    rows = []
    price = start
...
def test_crypto_explosion_armed_is_not_market_buy(monkeypatch):
    ...
    bars15 = _bars(60, start=9.42, step=0.009, volume=3000, last={
        "open": 9.92,
        "high": 9.98,
        "low": 9.88,
        "close": 9.95,
    ...
    bars4h = _bars(60, start=9.4, step=0.006, volume=5000)
...
def test_crypto_explosion_rejects_late_parabolic_move(monkeypatch):
    ...
    bars15 = _bars(60, start=8.0, step=0.035, volume=3000)
    bars4h = _bars(60, start=6.8, step=0.045, volume=5000)
    for row in bars4h[-5:]:
