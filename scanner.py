...
def load_common_stock_tickers_cached(api_key):
    ...
    try:
        ...
        for _ in range(20):
            if next_url:
                _resp = rate_limited_get(next_url, timeout=30)
            else:
                _resp = rate_limited_get(url, params=params, timeout=30)
...
def _early_progress_clear():
    ...
    except Exception as e: logging.warning(f"Failed to clear early progress file: {e}")

def _early_request_stop():
    try:
        with open(_EARLY_STOP_FILE, "w") as f: f.write("stop")
    except Exception as e: logging.warning(f"Failed to write early stop file: {e}")
...
def _early_clear_stop():
    ...
    except Exception as e: logging.warning(f"Failed to clear early stop file: {e}")
                ...
                if eval_results:
                    ...
                    for result in eval_results:
                        for ticker_data_item in tickers:
                            if ticker_data_item["ticker"] == result.get("ticker", 0):
    ...
    try:
        ...
        for ticker, name in contracts:
            try:
                ...
                quote = chart[0]
                meta = quote.get("meta", {})
                quotes = quote.get("indicators", {}).get("quote", [])
