...
def apply_presets(strat_name):
    # Diese Werte dienen als Basis-Leitplanken für die Filter
    presets = {
...
# --- 4. SCANNER KERN (MATHEMATISCHE BERECHNUNG) ---
if check_password():
        ...
        if st.session_state.scan_results:
            ...
            if selection.selection and selection.selection.rows:
                ...
                if new_symbol != st.session_state.selected_symbol:
                    ...
                    st.rerun()
        else: st.info("Bereit für Miroslav.")
    ...
    if st.button(f"🤖 GEMINI ANALYSE: {st.session_state.selected_symbol}"):
        with st.spinner("KI berechnet Sektoren-Rotation..."):
            prompt = f"Analysiere {st.session_state.selected_symbol}. Fokus auf Sektoren-Stärke und News-Sentiment."
