...
def apply_presets(strat_name, market_type):
    # Definition der mathematischen Filter für alle 13 Strategien
    presets = {
        "Volume Surge": {"RVOL": (2.0, 50.0), "Kursänderung %": (0.5, 30.0)},
        "Gap Momentum": {"Gap %": (2.5, 25.0), "RVOL": (1.2, 50.0)},
        "Penny Stock/Moon Shot": {"Preis": (0.0001, 5.0) if market_type == "Krypto" else (0.5, 5.0), "RVOL": (1.5, 50.0)},
        "Bull Flag": {"Vortag %": (4.0, 20.0), "Kursänderung %": (-2.0, 1.5), "RVOL": (1.1, 50.0)},
        "Unusual Volume": {"RVOL": (5.0, 100.0)},
        "High of Day (HOD)": {"Kursänderung %": (2.0, 30.0), "RVOL": (1.5, 50.0)},
        "Short Squeeze": {"Kursänderung %": (8.0, 50.0), "RVOL": (3.0, 100.0)},
        "Low Float": {"Kursänderung %": (10.0, 100.0), "RVOL": (2.0, 50.0)},
        "Blue Chip Pullback": {"Kursänderung %": (-5.0, -0.5), "SMA Trend %": (-5.0, 2.0)},
        "Multi-Day Runner": {"Vortag %": (3.0, 15.0), "Kursänderung %": (2.0, 15.0)},
        "Pre-Market Gapper": {"Gap %": (3.0, 40.0), "RVOL": (1.5, 50.0)},
        "Dead Cat Bounce": {"Vortag %": (-40.0, -10.0), "Kursänderung %": (1.0, 10.0)},
        "Golden Cross Proxy": {"SMA Trend %": (0.5, 15.0), "Kursänderung %": (0.5, 10.0)}
    }
    if strat_name in presets:
        st.session_state.active_filters = presets[strat_name].copy()

st.set_page_config(page_title="Alpha V35 Master", layout="wide")

# SIDEBAR (STRATEGIE & FILTER IN EINEM BLOCK)
with st.sidebar:
    st.title("💎 Alpha V35 Master")
    m_type = st.radio("Markt wählen:", ["Aktien", "Krypto"], horizontal=True)
    
    st.divider()
    # 1. Strategie wählen
    strat_list = ["Volume Surge", "Gap Momentum", "Penny Stock/Moon Shot", "Bull Flag", "Unusual Volume", "High of Day (HOD)", "Short Squeeze", "Low Float", "Blue Chip Pullback", "Multi-Day Runner", "Pre-Market Gapper", "Dead Cat Bounce", "Golden Cross Proxy"]
    ...
    # 2. Filter direkt darunter anzeigen und justieren
    st.subheader("⚙️ Filter-Feinjustierung")
    if st.session_state.active_filters:
        for name, values in list(st.session_state.active_filters.items()):
            new_val = st.slider(f"{name}", -100.0, 100.0 if "Vortag" in name or "Kurs" in name else 50.0, float(values[0]), float(values[1]), key=f"sl_{name}")
            st.session_state.active_filters[name] = new_val
    
    if st.button("🚀 SCAN STARTEN", type="primary", use_container_width=True):
        with st.status("Durchsuche Markt...") as status:
            ...
            try:
                ...
                for t in resp.get("tickers", []):
                    ...
                    # FILTER-LOGIK (Prüft JEDEN aktiven Filter)
                    match = True
...
with c_chart:
    st.subheader(f"📊 {st.session_state.selected_symbol} (4-Stunden Ansicht)")
...
# KI VOLLANALYSE
st.divider()
if st.button("🤖 VOLLANALYSE (News, S/R & Trendlinien)"):
    if "current_data" in st.session_state:
        with st.spinner("GPT-4o durchsucht News und berechnet Chart-Level..."):
            try:
                client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
                d = st.session_state.current_data
                prompt = f"Analysiere {d['Ticker']}: Preis {d['Price']}, RVOL {d['RVOL']}, Change {d['Chg%']}%. Nenne exakte Support/Resistance Level und prüfe News. Gib ein Rating 1-100."
                response = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}])
                st.markdown(f"### 🛡️ Professional Report: {d['Ticker']}")
                st.write(response.choices[0].message.content)
            except Exception as e: st.error(f"KI Fehler: {e}")
    else: st.warning("Wähle einen Ticker aus!")