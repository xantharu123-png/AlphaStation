...
def main() -> int:
    ...
    # Zellen: (scanner, "JJJJ-MM") -> {signals, r[], managed[]}
    cells: dict = defaultdict(lambda: {"signals": 0, "r": [], "managed": []})
    for row in rows:
        month = str(row.get("created_at") or "")[:7] or "unbekannt"
    ...
    header = (f"{'Monat':<9}{'Sig':>5}{'Entsch':>8}{'Win%':>7}{'KI95':>11}"
    ...
    for scanner in scanners + ["GESAMT"]:
        ...
        for month in months:
            ...
            print(f"{month:<9}{cell['signals']:>5}{decided:>8}{win_pct:>7}"
                  f"{ci:>11}{avg_r:>8}{avg_m:>9}{sum_r:>9}  {note}")
        print()

    print("Lies: Kippte die Quote in einem bestimmten Monat (Regime-Wechsel), "
