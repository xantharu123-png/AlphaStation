...
def test_be_after_mfe_rescues_loser_to_zero():
    assert st.simulate_breakeven_after_mfe(_row(max_favorable_r=1.2, r_realized=-1.0, outcome_detail="", tp1_hit_at="", status="STOP_HIT")) == 0.0
...
def test_be_after_mfe_keeps_winner():
    assert st.simulate_breakeven_after_mfe(_row(r_realized=0.8)) == 0.8
...
def test_be_after_mfe_custom_trigger():
    assert st.simulate_breakeven_after_mfe(_row(max_favorable_r=0.6, r_realized=-0.4), 0.5) == 0.0
...
def test_5050_be_falls_back_to_be_rule_without_tp1():
    ...
    assert st.simulate_managed_5050_breakeven(row) == 0.0


def test_5050_be_without_tp1_and_low_mfe_unchanged():
    row = _row(tp1_hit_at="", status="STOP_HIT", r_realized=-1.0, max_favorable_r=0.4)
